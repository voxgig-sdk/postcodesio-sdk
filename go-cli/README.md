...
boru-driven command-line client **and** interactive REPL for the Postcodesio
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/postcodesio-cli)
make build

# 2. See usage (words, entities, env vars)
./postcodesio-cli --help

# 3. Provide credentials once, via the environment
export POSTCODESIO_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./postcodesio-cli list nearest
./postcodesio-cli list outcode

# 5. Override the API base URL for a single call
POSTCODESIO_BASE=https://api.example.com ./postcodesio-cli list nearest

# 6. No arguments -> interactive REPL
./postcodesio-cli
postcodesio> list nearest
postcodesio> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/postcodesio-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/postcodesio-cli list nearest
   ```
...
Run with no arguments to open a REPL (prompt `postcodesio>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `nearest`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/postcodesio-cli`. |
...
(`./postcodesio-cli <expr>`) and interactively (the REPL), and expressions compose the
