...
type TerminatedPostcodeEntity struct {
	name    string
...
func NewTerminatedPostcodeEntity(client *core.PostcodesioSDK, entopts map[string]any) *TerminatedPostcodeEntity {
	if entopts == nil {
		entopts = map[string]any{}
	}
...
func (e *TerminatedPostcodeEntity) GetName() string { return e.name }
...
func (e *TerminatedPostcodeEntity) MarkDeleted() {
	e.deleted = true
}


// Deleted reports whether a successful Remove has resolved on this instance.
func (e *TerminatedPostcodeEntity) Deleted() bool {
	return e.deleted
}


func (e *TerminatedPostcodeEntity) Make() core.Entity {
	opts := map[string]any{}
	for k, v := range e.entopts {
		opts[k] = v
	}
	return NewTerminatedPostcodeEntity(e.client, opts)
}

func (e *TerminatedPostcodeEntity) Data(args ...any) any {
...
func (e *TerminatedPostcodeEntity) Match(args ...any) any {
	if len(args) > 0 && args[0] != nil {
...
// argument it returns the current data as an TerminatedPostcode; with an argument it
// sets the data and returns the stored value. It delegates to the untyped Data
// (identical runtime) and converts at the typed boundary.
func (e *TerminatedPostcodeEntity) DataTyped(data ...TerminatedPostcode) TerminatedPostcode {
	if len(data) > 0 {
		return typedFrom[TerminatedPostcode](e.Data(asMap(data[0])))
	}
	return typedFrom[TerminatedPostcode](e.Data())
}

// MatchTyped mirrors DataTyped for the entity's match filter. The match is a
// partial of the entity, so it round-trips through TerminatedPostcode (all fields
// optional at the wire level).
func (e *TerminatedPostcodeEntity) MatchTyped(match ...TerminatedPostcode) TerminatedPostcode {
	if len(match) > 0 {
		return typedFrom[TerminatedPostcode](e.Match(asMap(match[0])))
	}
	return typedFrom[TerminatedPostcode](e.Match())
}
...
func (e *TerminatedPostcodeEntity) Stream(action string, args map[string]any, callopts map[string]any) <-chan any {
	out := make(chan any)
...
func (e *TerminatedPostcodeEntity) Load(reqmatch map[string]any, ctrl map[string]any) (any, error) {
	utility := e.utility
...
// TerminatedPostcodeLoadMatch and returns an TerminatedPostcode. It delegates to the untyped
// Load (identical runtime) and converts at the typed boundary.
func (e *TerminatedPostcodeEntity) LoadTyped(reqmatch TerminatedPostcodeLoadMatch, ctrl map[string]any) (TerminatedPostcode, error) {
	res, err := e.Load(asMap(reqmatch), ctrl)
	if err != nil {
		return TerminatedPostcode{}, err
	}
	return typedFrom[TerminatedPostcode](res), nil
}



func (e *TerminatedPostcodeEntity) List(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("list", e.name)
}


func (e *TerminatedPostcodeEntity) Create(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("create", e.name)
}


func (e *TerminatedPostcodeEntity) Update(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("update", e.name)
}


func (e *TerminatedPostcodeEntity) Remove(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("remove", e.name)
}


func (e *TerminatedPostcodeEntity) runOp(ctx *core.Context, postDone func()) (any, error) {
	utility := e.utility
