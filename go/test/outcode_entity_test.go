...
func TestOutcodeEntity(t *testing.T) {
	t.Run("instance", func(t *testing.T) {
		testsdk := sdk.TestSDK(nil, nil)
		ent := testsdk.Outcode(nil)
		if ent == nil {
			t.Fatal("expected non-nil OutcodeEntity")
		}
	})

	t.Run("basic", func(t *testing.T) {
		setup := outcodeBasicSetup(nil)
		...
		for _, _op := range []string{"load"} {
			if _shouldSkip, _reason := isControlSkipped("entityOp", "outcode." + _op, _mode); _shouldSkip {
				if _reason == "" {
		...
		outcodeRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath("existing.outcode", setup.data)))
		var outcodeRef01Data map[string]any
		if len(outcodeRef01DataRaw) > 0 {
			outcodeRef01Data = core.ToMapAny(outcodeRef01DataRaw[0][1])
		}
...
func outcodeBasicSetup(extra map[string]any) *entityTestSetup {
	loadEnvLocal()

	_, filename, _, _ := runtime.Caller(0)
	dir := filepath.Dir(filename)

	entityDataFile := filepath.Join(dir, "..", "..", ".sdk", "test", "entity", "outcode", "OutcodeTestData.json")

	entityDataSource, err := os.ReadFile(entityDataFile)
	if err != nil {
		panic("failed to read outcode test data: " + err.Error())
	}

	var entityData map[string]any
	if err := json.Unmarshal(entityDataSource, &entityData); err != nil {
		panic("failed to parse outcode test data: " + err.Error())
	}
	...
	idmap := vs.Transform(
		[]any{"outcode01", "outcode02", "outcode03"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("POSTCODESIO_TEST_OUTCODE_ENTID")
	...
	idmapResolved := core.ToMapAny(env["POSTCODESIO_TEST_OUTCODE_ENTID"])
