...
import (
	...
	omni "github.com/voxgig-sdk/postcodesio-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.PostcodesioError); is {
				out := map[string]any{
					"name":    "PostcodesioError",
...
func omniCtx(arg any, client *sdk.PostcodesioSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.PostcodesioSDK); is {
			client = live
