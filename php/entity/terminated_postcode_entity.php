...
// Postcodesio SDK TerminatedPostcode entity
...
class TerminatedPostcodeEntity
{
    ...
    public function __construct($client, ?array $entopts = null)
    {
        ...
        $this->_name = "terminated_postcode";
        $this->_client = $client;
    ...
    {
        $opts = $this->_entopts;
        return new TerminatedPostcodeEntity($this->_client, $opts);
    }

    /**
     * @param TerminatedPostcode|array $args TerminatedPostcode data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = PostcodesioHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return TerminatedPostcode|array The current TerminatedPostcode data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of TerminatedPostcode fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = PostcodesioHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of TerminatedPostcode fields).
     */
     ...
     *
     * @param TerminatedPostcodeLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed TerminatedPostcodeLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return TerminatedPostcode|array The loaded TerminatedPostcode as an assoc-array at the
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = PostcodesioHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
