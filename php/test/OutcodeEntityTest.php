...
class OutcodeEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = PostcodesioSDK::test(null, null);
        $ent = $testsdk->Outcode(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = outcode_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "outcode." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set POSTCODESIO_TEST_OUTCODE_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $outcode_ref01_data = null;
        if (count($outcode_ref01_data_raw) > 0) {
...
function outcode_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/outcode/OutcodeTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["outcode01", "outcode02", "outcode03"] as $k) {
    ...
    $entid_env_raw = getenv("POSTCODESIO_TEST_OUTCODE_ENTID");
