...
type Args struct {
	Entity string         `json:"entity" jsonschema:"nearest | outcode | place | postcode | scottish_postcode | terminated_postcode"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.PostcodesioSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "postcodesio_list",
		Description: "List records from Postcodesio. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "postcodesio_load",
		Description: "Load a single record from Postcodesio. " +
...
func runOp(client *sdk.PostcodesioSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.PostcodesioSDK, name string) (sdk.PostcodesioEntity, error) {
	switch strings.ToLower(name) {
	case "nearest":
		return client.Nearest(nil), nil
	case "outcode":
		return client.Outcode(nil), nil
	case "place":
		return client.Place(nil), nil
	case "postcode":
		return client.Postcode(nil), nil
	case "scottish_postcode":
		return client.ScottishPostcode(nil), nil
	case "terminated_postcode":
		return client.TerminatedPostcode(nil), nil
