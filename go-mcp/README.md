...
[MCP](https://modelcontextprotocol.io) server exposing the Postcodesio SDK as
two agent tools — `postcodesio_list` and `postcodesio_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/postcodesio-mcp)
make build

# 2. Provide credentials via the environment
export POSTCODESIO_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user postcodesio \
  -- /absolute/path/to/postcodesio-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./postcodesio-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// postcodesio_list: first page of records
{ "entity": "nearest" }
{ "entity": "nearest", "query": { } }

// postcodesio_load: one record by id
{ "entity": "outcode", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/postcodesio-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user postcodesio \
     -- "$PWD"/dist/*/postcodesio-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `postcodesio_list` and `postcodesio_load` tools now appear
   in new sessions. Ask the agent to *"list nearest using postcodesio"*
   and it calls `postcodesio_list` with `{"entity":"nearest"}`.
...
```sh
export POSTCODESIO_APIKEY=sk_live_xxx            # API key
export POSTCODESIO_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `postcodesio_list` tool
...
### Call the `postcodesio_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `postcodesio_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `postcodesio_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `POSTCODESIO_APIKEY` | API key sent with every request. |
| `POSTCODESIO_BASE` | Optional override of the API base URL. |

### Entities

The 6 entities valid as the `entity` argument:

nearest | outcode | place | postcode | scottish_postcode | terminated_postcode

### Smoke test via HTTP (raw JSON-RPC)

```sh
./postcodesio-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"postcodesio_load","arguments":{"entity":"outcode","query":{"id":1}}}}'
```
