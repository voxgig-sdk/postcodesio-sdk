...
//     (\`new PostcodesioSDK(...)\` / \`PostcodesioSDK.test(...)\` become
//     \`PostcodesioSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"nearest":{"test01":{"id":"test01"}},"outcode":{"test01":{"id":"test01"}},"place":{"test01":{"id":"test01"}},"postcode":{"test01":{"id":"test01"}},"scottish_postcode":{"test01":{"id":"test01"}},"terminated_postcode":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new PostcodesioSDK(...)\` and any
// \`PostcodesioSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+PostcodesioSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/PostcodesioSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
