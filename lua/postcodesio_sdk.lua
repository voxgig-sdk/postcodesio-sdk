...
-- by passing it.
function PostcodesioSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function PostcodesioSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "PostcodesioSDK: graphql: " .. msg
    res.graphql = errors
  end
...
-- Idiomatic facade: client:ScottishPostcode():list() / client:ScottishPostcode():load({ id = ... })
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
...
-- Idiomatic facade: client:TerminatedPostcode():list() / client:TerminatedPostcode():load({ id = ... })
-- Entity access is capitalised (PascalCase) for parity with the other SDKs.
