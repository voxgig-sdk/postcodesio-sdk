...
describe("PostcodeDirect", function()
  it("should direct-list-postcode", function()
    local setup = postcode_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-postcode", setup.live and "live" or "unit")
  ...
  it("should direct-load-postcode", function()
    local setup = postcode_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-postcode", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function postcode_direct_setup(mockres)
