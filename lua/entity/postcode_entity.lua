-- Postcodesio SDK Postcode entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local PostcodeEntity = {}
PostcodeEntity.__index = PostcodeEntity


function PostcodeEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PostcodeEntity)
  self._name = "postcode"
  self._client = client
...
function PostcodeEntity:get_name()
  return self._name
end


function PostcodeEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PostcodeEntity.new(self._client, opts)
end
...
function PostcodeEntity:mark_deleted()
  self._deleted = true
end


function PostcodeEntity:deleted()
  return true == self._deleted
end


function PostcodeEntity:data_set(args)
  if args ~= nil then
...
function PostcodeEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PostcodeEntity:match_set(args)
  if args ~= nil then
...
function PostcodeEntity:match_get()
...
function PostcodeEntity:stream(action, args, callopts)
...
function PostcodeEntity:_run_op(ctx, post_done)
  local utility = self._utility
