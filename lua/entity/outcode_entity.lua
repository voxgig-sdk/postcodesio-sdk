...
local OutcodeEntity = {}
OutcodeEntity.__index = OutcodeEntity


function OutcodeEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, OutcodeEntity)
  self._name = "outcode"
  self._client = client
...
function OutcodeEntity:get_name()
  return self._name
end


function OutcodeEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return OutcodeEntity.new(self._client, opts)
end
...
function OutcodeEntity:mark_deleted()
  self._deleted = true
end


function OutcodeEntity:deleted()
  return true == self._deleted
end


function OutcodeEntity:data_set(args)
  if args ~= nil then
...
function OutcodeEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function OutcodeEntity:match_set(args)
  if args ~= nil then
...
function OutcodeEntity:match_get()
...
function OutcodeEntity:stream(action, args, callopts)
...
function OutcodeEntity:_run_op(ctx, post_done)
  local utility = self._utility
