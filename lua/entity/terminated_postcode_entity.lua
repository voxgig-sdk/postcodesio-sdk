...
local TerminatedPostcodeEntity = {}
TerminatedPostcodeEntity.__index = TerminatedPostcodeEntity


function TerminatedPostcodeEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, TerminatedPostcodeEntity)
  self._name = "terminated_postcode"
  self._client = client
...
function TerminatedPostcodeEntity:get_name()
  return self._name
end


function TerminatedPostcodeEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return TerminatedPostcodeEntity.new(self._client, opts)
end
...
function TerminatedPostcodeEntity:mark_deleted()
  self._deleted = true
end


function TerminatedPostcodeEntity:deleted()
  return true == self._deleted
end


function TerminatedPostcodeEntity:data_set(args)
  if args ~= nil then
...
function TerminatedPostcodeEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function TerminatedPostcodeEntity:match_set(args)
  if args ~= nil then
...
function TerminatedPostcodeEntity:match_get()
...
function TerminatedPostcodeEntity:stream(action, args, callopts)
...
function TerminatedPostcodeEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return TerminatedPostcodeEntity
