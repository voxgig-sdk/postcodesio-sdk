...
local NearestEntity = {}
NearestEntity.__index = NearestEntity


function NearestEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, NearestEntity)
  self._name = "nearest"
  self._client = client
...
function NearestEntity:get_name()
  return self._name
end


function NearestEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return NearestEntity.new(self._client, opts)
end
...
function NearestEntity:mark_deleted()
  self._deleted = true
end


function NearestEntity:deleted()
  return true == self._deleted
end


function NearestEntity:data_set(args)
  if args ~= nil then
...
function NearestEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function NearestEntity:match_set(args)
  if args ~= nil then
...
function NearestEntity:match_get()
...
function NearestEntity:stream(action, args, callopts)
...
function NearestEntity:_run_op(ctx, post_done)
  local utility = self._utility
