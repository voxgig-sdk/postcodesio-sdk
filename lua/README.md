...
The Lua SDK for the Postcodesio API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Nearest(nil):list(nil, nil)
...
local result, err = client:Postcodesio(nil):load(
  { id = "test01" }, nil
)
