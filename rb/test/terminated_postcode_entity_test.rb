...
class TerminatedPostcodeEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = terminated_postcode_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "terminated_postcode." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set POSTCODESIO_TEST_TERMINATED_POSTCODE_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    terminated_postcode_ref01_data = nil
    if terminated_postcode_ref01_data_raw.length > 0
...
def terminated_postcode_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "terminated_postcode", "TerminatedPostcodeTestData.json")
  ...
  entid_env_raw = ENV["POSTCODESIO_TEST_TERMINATED_POSTCODE_ENTID"]
