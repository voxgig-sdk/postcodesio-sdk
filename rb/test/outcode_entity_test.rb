...
class OutcodeEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = outcode_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "outcode." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set POSTCODESIO_TEST_OUTCODE_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    outcode_ref01_data = nil
    if outcode_ref01_data_raw.length > 0
    ...
    outcode_ref01_data_dt0_loaded, err = outcode_ref01_ent.load(outcode_ref01_match_dt0, nil)
    assert_nil err
...
def outcode_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "outcode", "OutcodeTestData.json")
  ...
  entid_env_raw = ENV["POSTCODESIO_TEST_OUTCODE_ENTID"]
