...
class ScottishPostcodeEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = scottish_postcode_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "scottish_postcode." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set POSTCODESIO_TEST_SCOTTISH_POSTCODE_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    scottish_postcode_ref01_data = nil
    if scottish_postcode_ref01_data_raw.length > 0
    ...
    scottish_postcode_ref01_data_dt0_loaded, err = scottish_postcode_ref01_ent.load(scottish_postcode_ref01_match_dt0, nil)
    assert_nil err
...
def scottish_postcode_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "scottish_postcode", "ScottishPostcodeTestData.json")
  ...
  entid_env_raw = ENV["POSTCODESIO_TEST_SCOTTISH_POSTCODE_ENTID"]
