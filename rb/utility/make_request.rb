...
module PostcodesioUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = PostcodesioResponse.new({})
    ...
    if fetch_err
      ...
      response = PostcodesioResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = PostcodesioResponse.new(fetched)
    else
