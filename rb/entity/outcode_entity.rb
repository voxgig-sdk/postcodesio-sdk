...
class OutcodeEntity
  ...
  def make
    opts = @_entopts.dup
    OutcodeEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = PostcodesioHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Outcode, Hash] the current Outcode data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = PostcodesioHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Outcode fields)
  def match_get
  ...
  #
  # @param reqmatch [OutcodeLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Outcode.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Outcode, Hash] the loaded Outcode; raises PostcodesioError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = PostcodesioHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
