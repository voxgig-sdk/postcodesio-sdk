...
class PostcodeEntity
  ...
  def make
    opts = @_entopts.dup
    PostcodeEntity.new(@_client, opts)
  end
  ...
  #   as an empty match, so client.Postcode.load works with no args).
  ...
  #
  # @param reqmatch [PostcodeListMatch, Hash, nil] match filter (any subset of
  #   Postcode fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
