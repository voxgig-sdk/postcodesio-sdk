...
The Python SDK for the Postcodesio API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Nearest(None).list(None, None)
if err:
    raise Exception(err)
...
result, err = client.Postcodesio(None).load(
    {"id": "test01"}, None
)
