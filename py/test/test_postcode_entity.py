...
class TestPostcodeEntity:

    def test_should_create_instance(self):
        testsdk = PostcodesioSDK.test(None, None)
        ent = testsdk.Postcode(None)
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Postcode(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Postcode(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _postcode_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create", "list", "load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "postcode." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set POSTCODESIO_TEST_POSTCODE_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        postcode_ref01_ent = client.Postcode(None)
...
def _postcode_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/postcode/PostcodeTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("POSTCODESIO_TEST_POSTCODE_ENTID"))
