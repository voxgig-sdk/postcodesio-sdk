...
class TestNearestDirect:

    def test_should_direct_list_nearest(self):
        setup = _nearest_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-nearest", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
        if setup["live"]:
            for _live_key in ["postcode01"]:
...
def _nearest_direct_setup(mockres):
