...
class TestScottishPostcodeDirect:

    def test_should_direct_load_scottish_postcode(self):
        setup = _scottish_postcode_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-scottish_postcode", "live" if setup["live"] else "unit")
        if _skip:
...
def _scottish_postcode_direct_setup(mockres):
