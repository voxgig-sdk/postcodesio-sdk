...
class TestNearestEntity:

    def test_should_create_instance(self):
        testsdk = PostcodesioSDK.test(None, None)
        ent = testsdk.Nearest(None)
    ...
    def test_should_stream(self):
        ...
        base = PostcodesioSDK.test(seed, None)
        seen = list(base.Nearest(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            sdk = PostcodesioSDK.test(
                seed, {"feature": {"streaming": {"active": True}}})
            got = []
            for item in sdk.Nearest(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _nearest_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["list"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "nearest." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set POSTCODESIO_TEST_NEAREST_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        nearest_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.nearest")))
        nearest_ref01_data = None
        if len(nearest_ref01_data_raw) > 0:
            nearest_ref01_data = helpers.to_map(nearest_ref01_data_raw[0][1])

        # LIST
        nearest_ref01_ent = client.Nearest(None)
        nearest_ref01_match = {
            "postcode_id": setup["idmap"]["postcode01"],
        }

        nearest_ref01_list_result = nearest_ref01_ent.list(nearest_ref01_match, None)
        assert isinstance(nearest_ref01_list_result, list)



def _nearest_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/nearest/NearestTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("POSTCODESIO_TEST_NEAREST_ENTID"))
