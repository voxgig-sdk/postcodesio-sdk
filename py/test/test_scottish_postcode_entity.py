...
class TestScottishPostcodeEntity:

    def test_should_create_instance(self):
        testsdk = PostcodesioSDK.test(None, None)
        ent = testsdk.ScottishPostcode(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _scottish_postcode_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "scottish_postcode." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set POSTCODESIO_TEST_SCOTTISH_POSTCODE_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        scottish_postcode_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.scottish_postcode")))
        scottish_postcode_ref01_data = None
        if len(scottish_postcode_ref01_data_raw) > 0:
        ...
        scottish_postcode_ref01_data_dt0_loaded, err = scottish_postcode_ref01_ent.load(scottish_postcode_ref01_match_dt0, None)
        assert err is None
        assert scottish_postcode_ref01_data_dt0_loaded is not None



def _scottish_postcode_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/scottish_postcode/ScottishPostcodeTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("POSTCODESIO_TEST_SCOTTISH_POSTCODE_ENTID"))
