...
class PostcodesioTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            PostcodesioTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = PostcodesioTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = PostcodesioTestRunner.getenv("POSTCODESIO_TEST_LIVE")
        override = PostcodesioTestRunner.getenv("POSTCODESIO_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = PostcodesioTestRunner.getenv(key)
        ...
        explain = PostcodesioTestRunner.getenv("POSTCODESIO_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["POSTCODESIO_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if PostcodesioTestRunner._test_control is not None:
            return PostcodesioTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                PostcodesioTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            PostcodesioTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = PostcodesioTestRunner.load_test_control()
...
def load_env_local():
    PostcodesioTestRunner.load_env_local()


def env_override(m):
    return PostcodesioTestRunner.env_override(m)


def entity_data(v):
    return PostcodesioTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return PostcodesioTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return PostcodesioTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return PostcodesioTestRunner.load_test_control()


def live_client_options():
    return PostcodesioTestRunner.live_client_options()


def live_delay_ms():
    return PostcodesioTestRunner.live_delay_ms()
