...
class TerminatedPostcodeEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return TerminatedPostcodeEntity(self._client, opts)
    ...
    def data_get(self) -> TerminatedPostcode:
    ...
    def match_get(self) -> TerminatedPostcode:
    ...
    def load(self, reqmatch=None, ctrl=None) -> TerminatedPostcode:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.TerminatedPostcode().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
