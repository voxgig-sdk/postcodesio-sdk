...
module PostcodesioUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = PostcodesioContext.new({}, nil)
    end
    op = ctx.op || PostcodesioOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || PostcodesioResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(PostcodesioError) ? err.msg : err.to_s
    ...
    sdk_err = PostcodesioError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(PostcodesioError)
