...
class FtCtrl extends PostcodesioControl
{
...
class FtClient
{
    ...
    private PostcodesioUtility $utility;

    public function __construct(PostcodesioUtility $utility, array $options)
    {
    ...
    public function get_utility(): PostcodesioUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (PostcodesioContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public PostcodesioContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (PostcodesioContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = PostcodesioConfig::make_feature($name);
    ...
    private static function buildUrl(PostcodesioSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof PostcodesioError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new PostcodesioResponse($fetched);
        }
    ...
    private function fail(PostcodesioContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (PostcodesioContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
